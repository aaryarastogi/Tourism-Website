...
import { PNR } from "../Schema/Trains/PNRSchema.js";
...
export async function handlepnrPostData(req, res){
  try {
    const newPNR = new PNR(req.body);
    await newPNR.save();
    res.status(201).json({ message: "PNR added successfully", data: newPNR });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
}

export async function handlepnrGetData(req, res){
  try {
    const { pnrNumber } = req.params;
    const currpnr = await PNR.findOne({pnr: pnrNumber });
...
export async function handleLiveTrainPostData(req, res){
  try {
    const liveTrain = await LiveTrain.create(req.body);
    res.status(201).json(liveTrain);
  } catch (error) {
    res.status(500).json({ message: "Error saving live train data" });
  }
}

export async function handleLiveTrainGetData(req, res){
  try {
    const liveTrain = await LiveTrain.findOne({ "train.number": req.params.number });
    if (!liveTrain) return res.status(404).json({ message: "Train not found" });
    res.json(liveTrain);
  } catch (error) {
